...
int minlengthscs(string a , string b,int n,int m){
    int dp[n+1][m+1];
/*  Here to find min length shortest common supersequence we 
    need to first find length of lcs
*/
    //initialisation
    rep(i,0,n+1){
        dp[i][0] = 0;
    ...
    int minlength_scs = m+n - lengthlcs;
    return minlength_scs;
}
...
void Solve(){
    ...
    cin>>a>>b;
    n = a.size(),m = b.size();
    int lengthscs = minlengthscs(a,b,n,m);//minlengthShortestcommonsupersequence
    cout<<lengthscs;
}
