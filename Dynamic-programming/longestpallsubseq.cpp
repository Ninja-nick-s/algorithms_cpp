...
//to find length of longest pallindromic subsequence
int longestpallsubseq(string a,int n){
    int m = n;
    int dp[n+1][m+1];
    string b = a;
    reverse(a.begin(),a.end()); //we need to reverse the given string and then find lcs of it
    //initialisation
    rep(i,0,n+1){
        dp[i][0] = 0;
    }
...
void Solve(){
    int n,m;//cin>>n>>m;
    string a,b;
    cin>>a; //given a single string
    n = a.size();
    int lenlonpallsubseq = longestpallsubseq(a,n);
    cout<<lenlonpallsubseq;
}
