...
int unbounded_knapsack(int val[],int wt[],int W,int n){   // 0/1 knapsack problem
    if( n==0 || W==0 ){
        return 0;           //base case
    }
    ...
    if( wt[n-1] <= W ){
        return dp[n][W] = max(
                val[n-1] + unbounded_knapsack(val,wt,W-wt[n-1],n) , //including the value
                unbounded_knapsack(val,wt,W,n-1)         //for not including the value & weight
            ) ;
    }
    else{
        return dp[n][W] = unbounded_knapsack(val,wt,W,n-1);  //this if for processed array
    }

}

int unbounded_knapsack_tab(int val[],int wt[],int W, int n){    //tabular way of solving knapsack problem
...
void Solve(){
    ...
    int res_rec = unbounded_knapsack(val,wt,W,n);

    int res_tab = unbounded_knapsack_tab(val,wt,W,n);
