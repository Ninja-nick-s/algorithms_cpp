...
int countsubsetsum(int a[],int sum,int n){  //counting no. of subsets with given sum

    for(int i = 0;i<=n;i++){    //initialisation or if recursion then its base case
        dp[i][0] = 1;
    }
    for(int i = 1;i<=sum;i++){
        dp[0][i] = 0;
...
void Solve(){
    int n,sum;cin>>n>>sum;
    int a[n];IA(a,n);
    int res = countsubsetsum(a,sum,n);
    cout<<res;
}
